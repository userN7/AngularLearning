	...
	public static class IdentitySeedData
	{
		...
		public static async Task SeedDatabase(IServiceProvider provider) {
			...
			if (role == null)
			{
				...
				if (!result.Succeeded) { throw new Exception($"Cannot create role: {result.Errors.FirstOrDefault()}");
				};				
			}
			...
			if (! await userManager.IsInRoleAsync(user, adminRole))
			{
				...
				if (!result.Succeeded)
				{
					throw new Exception($"Cannont add user to role: {result.Errors.FirstOrDefault()}");
				}
			}
		}
	}
